# What is the greatest product of adjacent numbers of the grid provided
...
# Horizontal - Test each x to x+3
for y in range(0,20):
	for x in range(0,17):
		currentval = grid[y][x] * grid[y][x+1] * grid[y][x+2] * grid[y][x+3]
		maxval = max(currentval,maxval)

# Vertical - Test each y to y+3
for y in range(0,16):
	for x in range(0,20):
		currentval = grid[y][x] * grid[y+1][x] * grid[y+2][x] * grid[y+3][x]
		maxval = max(currentval,maxval)

# Diagonal - Test both x ascending and x descending against y
for y in range(0,16):
	for x in range(0,16):
		currentval = grid[y][x] * grid[y+1][x+1] * grid[y+2][x+2] * grid[y+3][x+3]
		maxval = max(currentval,maxval)
for y in range(0,16):
	for x in range(0,16):
		currentval = grid[y][x+3] * grid[y+1][x+2] * grid[y+2][x+1] * grid[y+3][x]
		maxval = max(currentval,maxval)
